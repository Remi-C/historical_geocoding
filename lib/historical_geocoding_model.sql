...
-- LEAVE THIS TABLE EMPTY, INHERIT FROM IT OT USE IT, then create foreign key with function 
	...
	SELECT geohistorical_object.enable_disable_geohistorical_object( 'historical_geocoding', 'precise_localisation'::regclass,true) ; 
		-- example of index, not necessary here because this table will remain empty
		--geometric index
			CREATE INDEX ON historical_geocoding.precise_localisation USING GIST(geom) ; 
		--semantic index
			CREATE INDEX ON historical_geocoding.precise_localisation USING  GIN (normalised_name gin_trgm_ops);
		-- temporal index
			CREATE INDEX ON historical_geocoding.precise_localisation USING GIST(CAST(specific_fuzzy_date AS geometry)) ; 
		--index on source and origin
			CREATE INDEX ON historical_geocoding.precise_localisation (historical_source) ; 
			CREATE INDEX ON historical_geocoding.precise_localisation (numerical_origin_process) ; 
		-- index on associated_normalised_rough_name
			CREATE INDEX ON historical_geocoding.precise_localisation USING  GIN (associated_normalised_rough_name gin_trgm_ops);
